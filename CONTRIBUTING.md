...
- **What auto-merge is gated on.** Only the seven required contexts listed under "`main` is protected" below. The agent's `go`/`web` jobs are not among them yet, and the CodeQL checks prove the scan ran rather than that it was clean — [CLAUDE.md](CLAUDE.md) records both gaps and what closes them. Every ecosystem also carries a Dependabot publish cooldown, so nothing merges the day it is released.
